...
1. 確認 `output/case.json` 存在且關鍵欄位 `confidence` 均為 `high`（即已通過人工確認關卡）；否則退回 `/plan-intake`
...
寫入 `output/{案件名}-requirements.md`，標頭注明：法規版本、case.json 確認日期、規則庫中未核定（`verified: false`）參數的比例。
