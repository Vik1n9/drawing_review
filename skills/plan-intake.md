...
對 $ARGUMENTS（案件名）執行圖說結構化，產出 `case.json` 圖說底稿。這是整條審圖流水線的入口，**下游所有計算以本 skill 產出的 case.json 為準，不以 DXF、SVG 或圖片為準**。

## 輸入與輸出（統一資料夾）

- 輸入：`input/{案件名}/drawings/` 內的 DXF 圖面，搭配 `input/{案件名}/` 內的審查依據文件（只讀不改）
...
盤點 `input/{案件名}/drawings/` 內的 DXF 檔案與 `input/{案件名}/` 內的審查依據文件，先給出**圖說可讀性評級**並告知使用者：

| 等級 | 特徵 | 策略調整 |
|------|------|---------|
| A級（清晰完整） | DXF 圖層命名清楚，有樓層對應、圖例表、面積標注、審查依據文件 | 正常萃取，關鍵欄位仍須人工確認 |
| B級（部分可讀） | DXF 可解析但圖層/圖例不完整，部分樓層或設備符號需人工對照 | 可讀部分正常萃取；模糊欄位標 `confidence: low`，列入確認清單優先項 |
| C級（難以判讀） | DXF 缺檔、圖層混亂、座標或比例不明，或僅有掃描圖 | 轉為「人工輸入模式」：AI 只做欄位清單引導，數值全部由使用者提供 |
...
```json
{
  ...
  "source_drawings": [
    {
      "drawing_id": "1F",
      "path": "input/範例大樓/drawings/1F.dxf",
      "floor": "1F",
      "unit": "mm",
